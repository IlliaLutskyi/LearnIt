...
export async function generateSection(req: Request) {
  ...
  try {
    ...
    const prompt = `You are a section generator assistant. Based on this description ${data.prompt} generate a section with title ${data.title} **NOTE Lesson with ContentType Text is recommended over Lesson with ContentType Markdown, and Lesson with ContentType HighlightedCode is recommended for code examples or code snippets, and Text is tiptap format json **. Return the section in a JSON format. Response examples:
              ...
              items: {
                ...
                properties: {
                  ...
                  content: {
                    type: "string",
                    description:
                      "The content value. For 'Text' or 'Markdown' content, this is an HTML string. For 'Video', it's the embed URL. For 'HighlightedCode', it's the raw code string. For 'Table', it's a stringified JSON array of row objects.",
                  },
                  videoSource: {
                    type: "string",
                    description:
