...
export async function generateSection(req: Request) {
  ...
  try {
    ...
    const prompt = `You are a section generator assistant. Based on this description ${data.prompt} generate **NOTE Lesson with ContentType Text is the tiptap based html with StarterKit and Link extensions, Lesson with ContentType HighlightedCode is recommended for code examples or code snippets**. Return the section in a JSON format. Response examples:
